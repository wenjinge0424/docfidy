...
- (void)viewDidLoad {
    ...
    [self.inputToolbar setBackgroundColor:[UIColor whiteColor]];
...
- (void)refreshUI {
    ...
    NSString *name = @"";
    name = [NSString stringWithFormat:@"%@ %@", me[PARSE_USER_FIRSTNAME], me[PARSE_USER_LASTSTNAME]];
    if (name.length > 0)
...
- (NSString *) getDisplayName:(PFUser *)user {
    NSString *result = @"";
    NSString *name = @"";
    name = [NSString stringWithFormat:@"%@ %@", user[PARSE_USER_FIRSTNAME], user[PARSE_USER_LASTSTNAME]];
    if (name.length > 0)
        result = name;
    else
    ...
    else if (fileVideo)
    {
        ...
        if(filePhoto){
            [filePhoto getDataInBackgroundWithBlock:^(NSData * _Nullable data, NSError * _Nullable error) {
                if (!error) {
                    ((UIImageView*)mediaItem.mediaView).image = [UIImage imageWithData:data];
...
- (void)sendPushNotification:(NSString *)msg {
    ...
    if (isEnableChatNotify) {
        ...
        if(!me[PARSE_USER_FULLNAME] || ((NSString*)me[PARSE_USER_FULLNAME]).length == 0){
            name = [NSString stringWithFormat:@"%@ %@", me[PARSE_USER_FIRSTNAME], me[PARSE_USER_LASTSTNAME]];
        }
        NSString *pushMsg = [NSString stringWithFormat:@"%@ sent a message %@", name, msg];
        
        for(PFUser * subUser in self.toUsers){
            if(![subUser.objectId isEqualToString:me.objectId]){
                [subUser fetchIfNeeded];
                if(![subUser[PARSE_USER_DONOTDISCRIB] boolValue]){
                    NSDictionary *data = @{
                                           @"alert" : pushMsg,
                                           @"badge" : @"Increment",
                                           @"sound" : @"cheering.caf",
...
- (void) imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info{
    ...
    if ([type isEqualToString:(NSString *)kUTTypeMovie]){
        ...
        [self sendMessage:@"" video:[PFFile fileWithName:@"video.mov" data:videoData] photo:[PFFile fileWithData:data]];
    } else {
...
- (PFUser *) getSendUser:(NSString*)objectId
{
    for(PFUser * subUser in self.toUsers){
        if([subUser.objectId isEqualToString:objectId])
            return subUser;
    }
    return nil;
}
    ...
    if (!msg.isMediaMessage) {
        ...
        if ([msg.senderId isEqualToString:self.senderId]) {
            ...
            if (!me[PARSE_USER_AVATAR])
                cell.avatarImageView.image = [JSQMessagesAvatarImageFactory circularAvatarImage:[UIImage imageNamed:@"ic_default_avatar"] withDiameter:48];
            else{
            ...
            cell.textView.textColor = [UIColor whiteColor];
            PFUser * senderObj = [self getSendUser:msg.senderId];
            if (!senderObj[PARSE_USER_AVATAR])
                cell.avatarImageView.image = [JSQMessagesAvatarImageFactory circularAvatarImage:[UIImage imageNamed:@"ic_default_avatar"] withDiameter:48];
            else{
                [Util setImage:cell.avatarImageView imgFile:senderObj[PARSE_USER_AVATAR]];
            }
        }
